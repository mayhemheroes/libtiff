...
# WHETHER OR AN THEORY OF LIABILITY, ARISING OUT OF OR IN CONNECTION WITH
# THE USE OR PERFORMANCE OF THIS SOFTWARE.

# Clang 22 emits DWARF 5 by default; combine with -gline-tables-only breaks
# LLD and some linkers. Force DWARF 4 throughout.
...
# Build libjbig (uses plain make; pass CC so ASan instrumentation is consistent)
pushd "$SRC/jbigkit"
if [ "$ARCHITECTURE" = "i386" ]; then
    make lib CC="$CC -m32" CFLAGS="$CFLAGS"
else
...
# Build libtiff itself (cmake . runs from WORKDIR = $SRC/libtiff)
cmake . \
...
for fuzzer in "$SRC/libtiff/contrib/oss-fuzz/"*fuzzer.cc; do
    ...
    if [ "$ARCHITECTURE" = "i386" ]; then
        $CXX $CXXFLAGS -std=c++11 -I"$WORK/include" \
            ...
            "$WORK/lib/libz.a" "$WORK/lib/libjpeg.a" \
...
# Package seed corpus from AFL test cases
